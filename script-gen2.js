...
let timerInterval, timerStarted = false, secondsElapsed = parseInt(localStorage.getItem('timerGen2')) || 0;
...
function startTimer() {
    if (!timerStarted && scoreActuel < 100) {
        timerStarted = true;
        timerInterval = setInterval(() => { secondsElapsed++; timerText.innerText = formatTime(secondsElapsed); localStorage.setItem('timerGen2', secondsElapsed); }, 1000);
    }
}

window.addEventListener('beforeunload', (e) => { if (scoreActuel > 0 && scoreActuel < 100) { e.preventDefault(); e.returnValue = ''; } });
document.getElementById('btn-reset').addEventListener('click', () => { if(confirm("Recommencer à zéro ?")) { localStorage.removeItem('sauvegardeGen2'); localStorage.removeItem('timerGen2'); location.reload(); } });
...
async function chargerPokemons() {
    ...
    const requeteGraphQL = `query { pokemonspecies(where: {id: {_gte: 152, _lte: 251}}) { id name pokemonspeciesnames(where: {language_id: {_eq: 5}}) { name } pokemons { id } } }`;
    try {
...
function declencherVictoire() {
    new Audio('https://raw.githubusercontent.com/PokeAPI/cries/main/cries/pokemon/latest/172.ogg').play();
...
function validerPokemon(idPokemon, formes, nomSaisi, joueurActif = true) {
    ...
    if (!box.classList.contains('trouve')) {
        ...
        if(joueurActif) {
            ...
            pokemonsTrouves.push(idPokemon); localStorage.setItem('sauvegardeGen2', JSON.stringify(pokemonsTrouves));
        ...
        if (scoreActuel === 100 && joueurActif) { clearInterval(timerInterval); input.disabled = true; input.placeholder = "INCROYABLE ! FINI !"; declencherVictoire(); }
    }
}
