...
let timerInterval, timerStarted = false, secondsElapsed = parseInt(localStorage.getItem('timerGen3')) || 0;
...
async function chargerPokemons() {
    ...
    const requeteGraphQL = `query { pokemonspecies(where: {id: {_gte: 252, _lte: 386}}) { id name pokemonspeciesnames(where: {language_id: {_eq: 5}}) { name } pokemons { id } } }`;
    try {
...
function declencherVictoire() {
    new Audio('https://raw.githubusercontent.com/PokeAPI/cries/main/cries/pokemon/latest/384.ogg').play();
