...
let timerInterval, timerStarted = false, secondsElapsed = parseInt(localStorage.getItem('timerGen6')) || 0;
...
function startTimer() {
    if (!timerStarted && scoreActuel < 72) {
        timerStarted = true;
        timerInterval = setInterval(() => { secondsElapsed++; timerText.innerText = formatTime(secondsElapsed); localStorage.setItem('timerGen6', secondsElapsed); }, 1000);
    }
}

window.addEventListener('beforeunload', (e) => { if (scoreActuel > 0 && scoreActuel < 72) { e.preventDefault(); e.returnValue = ''; } });
document.getElementById('btn-reset').addEventListener('click', () => { if(confirm("Recommencer à zéro ?")) { localStorage.removeItem('sauvegardeGen6'); localStorage.removeItem('timerGen6'); location.reload(); } });
...
async function chargerPokemons() {
    ...
    const requeteGraphQL = `query { pokemonspecies(where: {id: {_gte: 650, _lte: 721}}) { id name pokemonspeciesnames(where: {language_id: {_eq: 5}}) { name } pokemons { id } } }`;
    try {
        ...
        const sauvegarde = localStorage.getItem('sauvegardeGen6');
        if (sauvegarde) {
...
function declencherVictoire() {
    new Audio('https://raw.githubusercontent.com/PokeAPI/cries/main/cries/pokemon/latest/716.ogg').play();
...
function validerPokemon(idPokemon, formes, nomSaisi, joueurActif = true) {
    ...
    if (!box.classList.contains('trouve')) {
        ...
        if(joueurActif) {
            ...
            pokemonsTrouves.push(idPokemon); localStorage.setItem('sauvegardeGen6', JSON.stringify(pokemonsTrouves));
        ...
        if (scoreActuel === 72 && joueurActif) { clearInterval(timerInterval); input.disabled = true; input.placeholder = "INCROYABLE ! FINI !"; declencherVictoire(); }
    }
}
