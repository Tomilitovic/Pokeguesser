...
let timerInterval, timerStarted = false, secondsElapsed = parseInt(localStorage.getItem('timerGen9')) || 0;
...
function startTimer() {
    if (!timerStarted && scoreActuel < 120) {
        timerStarted = true;
        timerInterval = setInterval(() => { secondsElapsed++; timerText.innerText = formatTime(secondsElapsed); localStorage.setItem('timerGen9', secondsElapsed); }, 1000);
    }
}

window.addEventListener('beforeunload', (e) => { if (scoreActuel > 0 && scoreActuel < 120) { e.preventDefault(); e.returnValue = ''; } });
document.getElementById('btn-reset').addEventListener('click', () => { if(confirm("Recommencer à zéro ?")) { localStorage.removeItem('sauvegardeGen9'); localStorage.removeItem('timerGen9'); location.reload(); } });
...
async function chargerPokemons() {
    ...
    const requeteGraphQL = `query { pokemonspecies(where: {id: {_gte: 906, _lte: 1025}}) { id name pokemonspeciesnames(where: {language_id: {_eq: 5}}) { name } pokemons { id } } }`;
    try {
        ...
        const sauvegarde = localStorage.getItem('sauvegardeGen9');
        if (sauvegarde) {
...
function declencherVictoire() {
    new Audio('https://raw.githubusercontent.com/PokeAPI/cries/main/cries/pokemon/latest/906.ogg').play();
...
function validerPokemon(idPokemon, formes, nomSaisi, joueurActif = true) {
    ...
    if (!box.classList.contains('trouve')) {
        ...
        if(joueurActif) {
            ...
            pokemonsTrouves.push(idPokemon); localStorage.setItem('sauvegardeGen9', JSON.stringify(pokemonsTrouves));
        ...
        if (scoreActuel === 120 && joueurActif) { clearInterval(timerInterval); input.disabled = true; input.placeholder = "INCROYABLE ! FINI !"; declencherVictoire(); }
    }
}
